...
export async function executeWrite(input: {
  ...
  for (const delay of [2000, 4000, 8000]) {
    input.progress({ phase: "WAITING_FOR_FINALITY", hash }); await wait(delay);
  ...
  try { await input.verify({ ...record, tx_hash: hash, status: "SUBMITTED" }); }
  catch (cause) { await updateRecord(record.reservation, { status: "RECONCILE" }); input.progress({ phase: "RECONCILIATION_REQUIRED", hash, message: cause instanceof Error ? cause.message : "Readback mismatch." }); return; }
