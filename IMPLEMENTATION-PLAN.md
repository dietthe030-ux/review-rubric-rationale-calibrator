...
- Vercel-release correction: the frontend uses the official `studioDevnet` chain exported by `genlayer-js@2.0.0-rc.1` for Studio Next chain `61997`; it does not redirect or mutate the stable `studionet` chain. A direct chain-config regression and live `get_count` probe cover the target. Public UI copy does not expose technical chain IDs, provider-routing terms or SDK/RPC implementation labels. Contract bytes and Studio evidence remain unchanged; frontend build, Vercel deployment and Vercel E2E must be rerun for this release.
