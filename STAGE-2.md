# Review Rubric Rationale Calibrator — Stage 2
...
Common storage/domain={}. Base `{dimensions:N(4,{id:ID,min:int,max:int,anchors:N(11,{score:int,text:T(96)})})}`. min/max in[-100,100], `0<=max-min<=10`, each integer min..max appears exactly once, ascending anchor order; reject duplicate/gap/extra scores before lock. Width10 allowed; width11 rejects. Response `{reviews:N(4,{dimension_id:ID,score:int,rationale:T(768)})}` same dimension order, every one exactly once, in-range score. Caps base8192,response4096. Result `{v:1,labels:N(4,'SUPPORTS_SELECTED'|'SUPPORTS_OTHER'|'MISSING_ANCHOR_REASON'|'UNKNOWN')}` exactly dimension order. Task: selected anchor is supported by rationale's expressed reasons, not sheer length; SUPPORTS_OTHER means explicit rationale fits an incompatible different score anchor; copying labels without reasoning=MISSING. UNKNOWN globally→UNRESOLVED; else OTHER→RATIONALE_SCORE_CONFLICT; else MISSING→ANCHOR_MISSING; else CALIBRATED. No hidden score rewrite.

| Public signature | Caller/profile | Postcondition |
|---|---|---|
| create_rubric(nonce:str,reviewer:Address,base_json:str,parent:u256)->u256 | any/CREATE distinct | BASE_DRAFT |
| replace_rubric(id:u256,base_json:str,expected_revision:u256)->None | primary/EDIT_BASE | validated anchors replaced |
| lock_rubric(id:u256,expected_revision:u256)->None | primary/LOCK_BASE | BASE_LOCKED |
| put_review(id:u256,response_json:str,expected_revision:u256)->None | secondary/PUT_RESPONSE | RESPONSE_DRAFT |
| freeze_review(id:u256,expected_revision:u256)->None | secondary/LOCK_RESPONSE | FROZEN |
| calibrate_review(id:u256,expected_revision:u256)->None | any/EVALUATE | DONE calibration/rework outcome or UNRESOLVED |
| retry_review(id:u256,expected_revision:u256)->None | any/RETRY | common |

UI owner dimension/range/anchor grid, explicit lock; reviewer bounded score input and separate rationale per dimension; calibration table labels. Copy `Rationale matches the selected rubric anchors` / `Review needs revision` / `Unresolved`; never `Submission quality verified`. Tests width10/11, missing middle/duplicate anchor, bool score, wrong reviewer, post-lock rubric mutation, generic rationale, contrary anchor, uncertainty, every write+view and pending switch case.

WRONG:
```python
assert len(anchors) <= 11  # does not ensure interval coverage
```
CORRECT:
```python
assert 0 <= high-low <= 10
assert [a['score'] for a in anchors] == list(range(low,high+1))
```
Symptom schema-valid but impossible rubric; cause missing range invariant; verify `pytest tests/test_contract.py -k anchors`; repaired when width10 complete passes and width11/gap/duplicate fail before consensus.
